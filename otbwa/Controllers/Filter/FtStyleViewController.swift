...
class FtStyleViewController: BaseViewController {
    ...
    static func initWithData(_ data:[JSON], _ configData:[JSON]?, completion:((_ item: Any?, _ action:Int) ->Void)?) -> FtStyleViewController {
        let vc = FtStyleViewController.instantiateFromStoryboard(.common)!
    ...
    override func viewDidLoad() {
        ...
        let attrN = NSAttributedString.init(string: "스타일 초기화", attributes: [.foregroundColor:RGB(160, 160, 160), .font: UIFont.systemFont(ofSize: 13, weight: .medium)])
        let attrS = NSAttributedString.init(string: "스타일 초기화", attributes: [.foregroundColor:UIColor.white, .font: UIFont.systemFont(ofSize: 13, weight: .medium)])
...
extension FtStyleViewController: UICollectionViewDataSource, UICollectionViewDelegate, CFlowLayoutDelegate {
