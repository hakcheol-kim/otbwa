...
class WProductModifyViewController: BaseViewController {
    ...
    var selTag: JSON! {
        didSet {
            self.tfCategory.text = selTag["name"].stringValue
        }
    }
    var images = [UIImage]() {
        didSet {
    ...
    func requestProductInfo() {
        ApiManager.ins.requestProductInfo(productNo:passData["product_no"].stringValue) { res in
            if res["success"].boolValue {
                self.data = res["data"]
                self.configuratinUi()
                print(self.data["aiTags"])
            }
            else {
    ...
    func configuratinUi() {
        ...
        let dispYN = data["dispYN"].stringValue
        let color_list = data["color_list"].arrayValue
        let tagList = data["tagList"].arrayValue
        let info = data["info"].stringValue
        let matchingYN = data["matchingYN"].stringValue
        let pieceYN = data["pieceYN"].stringValue
        let made_in = data["made_in"].stringValue
        
        let size = passData["option"]["size"].stringValue
        let color = passData["option"]["color"].stringValue
        tfSize.text = size
        tfColor.text = color
        
        tfProdName.text = name
        tfProdPrice.text = price.stringValue
        ...
        else if dispYN == "client" {
            btnClient.isEnabled = true
        }
        else if dispYN == "soldout" {
    ...
    @IBAction func onClickedBtnActions(_ sender: UIButton) {
        if sender == btnAddPhoto {
            ...
            param["name"] = name
            param["product_no"] = passData["product_no"].intValue
            ...
            ApiManager.ins.requestModifyProductInfo(param) { res in
                if res["success"].boolValue {
