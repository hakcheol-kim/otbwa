//
//  WCompOrderDetailViewController.swift
//  otbwa
//
...
import UIKit
import SwiftyJSON
class WCompOrderDetailViewController: BaseViewController {
    ...
    @IBOutlet weak var btnCalling: CButton!
    ...
    override func viewDidLoad() {
        ...
        CNavigationBar.drawTitle(self, "매장 주문", nil)
        
        requestCompOrderDetailInfo()
    }
    
    func requestCompOrderDetailInfo() {
        var param = [String:Any]()
        param["comp_no"] = ShareData.ins.compNo
        param["order_no"] = passData["order_no"].stringValue
        ApiManager.ins.requestCompOrderDetailInfo(param) { res in
    ...
    func decorationUi() {
        ...
        if let lb = orderNumView.viewWithTag(101) as? UILabel {
            ...
            lb.text = comp_nm+"/"+comp_nm_scd
        }
        
        if let lb = orderNumView.viewWithTag(102) as? UILabel {
...
//           "req_txt": "",
        ...
        if let lb = receiveMethod.viewWithTag(101) as? UILabel {
            let recipient = data["recipient"].stringValue
            lb.text = "수령인 : \(recipient)"
        }
        if let lb = receiveMethod.viewWithTag(102) as? UILabel {
            let recipient_tel = data["recipient_tel"].stringValue
            lb.text = "수령인 연락처 : \(recipient_tel)"
        }
        if let lb = receiveMethod.viewWithTag(103) as? UILabel {
            let rcp_addr = data["address"].stringValue
        ...
        let order = data["option"].arrayValue
    ...
    @IBAction func onClickedBtnActions(_ sender: UIButton) {
        if sender == btnCalling {
            let comp_tel = data["comp_tel"].stringValue
            guard comp_tel.isEmpty == false else {
                return
            }
            let url = "tel://\(comp_tel)"
