//
//  RTotalProductViewController.swift
//  otbwa
//
...
import UIKit
import SwiftyJSON
class RTotalProductViewController: BaseViewController {
...
//        headerView.configurationData(nil, filters)
    ...
    func requestEventBannerList() {
        ApiManager.ins.requestEventBannerList { res in
            let success = res["success"].boolValue
            if success {
                let list = res["data"]["list"].arrayValue
                self.baners.removeAll()
                self.baners.append(contentsOf: list)
    ...
    func requestAllProductList() {
        ...
        ApiManager.ins.requestAllProductList(param: param) { res in
...
extension RTotalProductViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, referenceSizeForHeaderInSection section: Int) -> CGSize {
        let bannerH:CGFloat = collectionView.bounds.size.width*(1.0/2.2)
        return .init(width: view.bounds.width, height: (bannerH + 20 + 48 + 20 + 30 + 8))
    }
...
extension RTotalProductViewController: UIScrollViewDelegate {
